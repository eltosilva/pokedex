...
export class PokeApi {
  ...
  getPokemons(offset = 0, limit = 10) {
    ...
    return fetch(`${url}?offset=${offset}&limit=${limit}`)
      ...
      .then(pokemons => pokemons.map(this.getPokemonsDetails))
      .then(promisePokemons => Promise.all(promisePokemons))
      .then(pokemonsDetails => pokemonsDetails.map(converteDetailsToPokemon))
  }
...
function converteDetailsToPokemon(pokemonDetail){
  const pokemon = new Pokemon()

  pokemon.number = pokemonDetail.order
  pokemon.name = pokemonDetail.name
  pokemon.types = pokemonDetail.types.map(obj => obj.type.name)
